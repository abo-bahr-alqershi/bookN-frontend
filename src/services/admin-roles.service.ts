...
export const AdminRolesService = {
  ...
  getAll: (query?: GetAllRolesQuery) =>
    axios.get<PaginatedResult<RoleDto>>('/api/admin/Roles', { params: query }).then(res => res.data),

  /** جلب دور بواسطة المعرف */
  getById: (roleId: string) =>
    axios.get<ResultDto<RoleDto>>(`/api/admin/Roles/${roleId}`).then(res => res.data),

  /** إنشاء دور جديد */
  create: (data: CreateRoleCommand) =>
    axios.post<ResultDto<string>>('/api/admin/Roles', data).then(res => res.data),

  /** تعديل دور */
  update: (roleId: string, data: UpdateRoleCommand) =>
    axios.put<ResultDto<boolean>>(`/api/admin/Roles/${roleId}`, data).then(res => res.data),

  /** حذف دور */
  delete: (roleId: string) =>
    axios.delete<ResultDto<boolean>>(`/api/admin/Roles/${roleId}`).then(res => res.data),
};
