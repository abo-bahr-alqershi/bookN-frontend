...
export const PropertyServicesService = {
  ...
  getByProperty: (query: GetPropertyServicesQuery) =>
    axios.get<ResultDto<ServiceDto[]>>(`${API_BASE}/property/${query.propertyId}`, { params: query }).then(res => res.data),

  /** جلب خدمة عقار بحسب المعرف */
  getById: (query: GetServiceByIdQuery) =>
    axios.get<ResultDto<ServiceDetailsDto>>(`${API_BASE}/${query.serviceId}`).then(res => res.data),
